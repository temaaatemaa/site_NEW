...
io.sockets.on('connection', function (client) {
    client.on('message', function (message) {
        try {
	        /*
	        if (message.substr(0, 3)=='eps')
	        {
		    eps=Number(message.slice(4))
		    message='eps = '+ eps + '-----OK';
		...
		//alert('Iters: ' + k + 'Epsilon: ' + eps + 'Root: ' + x0);
		    ...
		    client.broadcast.emit('message', message);
//var eps = $('input[name="EPS"]').val();
//var a = $('input[name="X0"]').val();
//var b = $('input[name="X1"]').val();
...
case '4': {
	...
	message='Метод хорд!';
		    ...
		    client.broadcast.emit('message', message);
	/**
 * Created by Диман on 04.11.2016.
 */
function equation(x) {
...
case '5':{
	message='Метод простой итерации!';
		    ...
		    client.broadcast.emit('message', message);
/**
 * Created by Диман on 05.11.2016.
 */
